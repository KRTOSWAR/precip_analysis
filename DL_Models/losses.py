...
def _tf_percentile(tensor, q):
    """
    q-th percentile of a tensor using tf.sort (no tfp needed).
...
def _fss_single_3d(yt_3d, yp_3d, q, n):
    ...
    and adds/removes the batch dimension internally for conv2d.
    """
    # Add batch dim: (H, W, 1) -> (1, H, W, 1)
    yt = tf.expand_dims(yt_3d, axis=0)
    ...
    # conv2d requires (batch, H, W, channels) — satisfied by the expand_dims
...
def fss_loss_batch(y_true, y_pred, q, n=C.FSS_N):
    """
    Mean FSS' across all samples in the batch.
    y_true, y_pred: (B, H, W, 1) — ocean pixels already zeroed before call.
    """
    # tf.map_fn iterates over axis-0, delivering (H, W, 1) per sample
    fss_vals = tf.map_fn(
        fn=lambda pair: _fss_single_3d(pair[0], pair[1], q, n),
        elems=(y_true, y_pred),
        fn_output_signature=tf.float32    # scalar output per sample
    )
...
def compound_loss(y_true, y_pred,
    ...
    """
    # Zero out ocean pixels once here — reused by all FSS calls
